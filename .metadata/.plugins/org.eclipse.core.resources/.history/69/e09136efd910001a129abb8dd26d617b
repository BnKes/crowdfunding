...
public class TestActiviti {
	...
	public void test12(){
		...
		for ( Task task : tasks ) {
			taskService.setVariable(task.getId(), "flag", false);  //设置变量值
	...
	public void test091(){
		...
		TaskService taskService = processEngine.getTaskService();
		TaskQuery taskQuery = taskService.createTaskQuery();
		List<Task> list0 = taskQuery.taskAssignee("zhangsan").list();
		for(Task task : list0){
			taskService.complete(task.getId());
		}
	...
	public void test08(){
		...
		ProcessInstance processInstance = runtimeService.startProcessInstanceById(processDefinition.getId(),varibales);//放入map
		System.out.println("processInstance="+processInstance);
	}
	//7.领取任务
	@Test
	...
	public void test05(){
		...
		for (Task task : list1) {
			...
			System.out.println("name="+task.getName());
			//zhangsan完成任务
			taskService.complete(task.getId());//完成任务，进行下一步
		}
		System.out.println("------------------------------------");
		//lisi的任务:
		System.out.println("lisi的任务:");
